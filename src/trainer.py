...
class Trainer(object):
    ...
    def get_iterator(self, iter_name, lang1, lang2, stream):
        ...
        if lang2 is None:
            ...
            if lang1 == lang2:
                iterator = self.data['diverse'][(_lang1, _lang2)]['train'].get_iterator(
                    shuffle=True,
                    group_by_size=self.params.group_by_size,
                    n_sentences=-1,
            )
...
class EncDecTrainer(Trainer):
    ...
    def mt_step(self, lang1, lang2, lambda_coeff, bleu_npy=None, dbleu_npy=None):
        """
        ...
        if lang1 == lang2 and lang1!='en':
            ...
            # x = self.get_batch('mt', lang1, lang2)
            # print("x", x)
            idxs, (x1, len1), (x2, len2) = self.get_batch('mt', lang1, lang2)        
        ...
        bleu_norm=None
        dbleu_norm=None
        if os.path.exists(bleu_npy):
            with open(bleu_npy, "rb") as f:
                bleus = pickle.load(f)
            bleus = np.array(bleus)
            bleu_norm = 100.0 - bleus[idxs]
        if os.path.exists(dbleu_npy):
            with open(dbleu_npy, "rb") as f:
                dbleus = pickle.load(f)
            dbleus = np.array(dbleus)
            dbleu_norm = 100.0 - dbleus[idxs]

        _, loss = self.decoder('predict', tensor=dec2, pred_mask=pred_mask, y=y, get_scores=False, bleu_norm=bleu_norm, dbleu_norm=dbleu_norm)
        self.stats[('AE-%s' % lang1) if lang1 == lang2 and lang1!='en' else ('MT-%s-%s' % (lang1, lang2))].append(loss.item())
        loss = lambda_coeff * loss
    ...
    def bt_step(self, lang1, lang2, lang3, lambda_coeff):
        ...
        with torch.no_grad():
            ...
            code_path = params.data_path+"/vocab.codes"
            x2, len2 = _decoder.generate_diverse(enc1, len1, lang2_id, max_len=int(1.3 * len1.max().item() + 5), sample_temperature=1, code_path=code_path)
            #remove the codes
            x2 = torch.cat((x2[0,:].unsqueeze(0),x2[2:,:])) #remove the codes in src
            len2-=1
