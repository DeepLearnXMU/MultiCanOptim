...
def load_diverse_data(params, data):
    """
    Load parallel data.
    """
    data['diverse'] = {}

    for src, tgt in params.diverse_dataset.keys():

        logger.info('============ Parallel data (%s-%s)' % (src, tgt))

        assert (src, tgt) not in data['diverse']
        data['diverse'][(src, tgt)] = {}
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            # load binarized datasets
            src_path, tgt_path = params.diverse_dataset[(src, tgt)][splt]
            src_data = load_binarized(src_path, params)
            tgt_data = load_binarized(tgt_path, params)
            ...
            data['diverse'][(src, tgt)][splt] = dataset
            logger.info("")
...
def load_data(params):
    ...
    for (src, tgt), v in data['diverse'].items():
        for data_set in v.keys():
            logger.info('{: <18} - {: >5} - {: >12}:{: >10}'.format('Parallel data (parabank2)', data_set, '%s-%s' % (src, tgt), len(v[data_set])))
