...
class Evaluator(object):
    ...
    def get_iterator(self, data_set, lang1, lang2=None, stream=False):
        ...
        if lang2 is None:
            ...
            if lang1==lang2:
                iterator = self.data['diverse'][(_lang1, _lang2)][data_set].get_iterator(
                    shuffle=False,
                    group_by_size=False,
                    n_sentences=n_sentences
                )
...
class EncDecEvaluator(Evaluator):
    ...
    def evaluate_mt(self, scores, data_set, lang1, lang2, eval_bleu, K=3):
        """
        Evaluate perplexity and next word prediction accuracy.
        K: the number of diverse tranlsations, K>0 will do diverse_mt
        """
        ...
        for batch in self.get_iterator(data_set, lang1, lang2):
            ...
            if eval_bleu:
                ...
                if K>0:
                    ...
                    generated_ks, lengths_ks, _ = decoder.generate_diverse_beam(
                        K, enc1, len1, lang2_id, beam_size=params.beam_size,
                    ...
                    #print("generated_ks", generated_ks[0].shape)
                    for i in range(K):
                        hypothesis_ks[i].extend(convert_to_diverse_text(generated_ks[i], lengths_ks[i], self.dico, params))
                else:
        ...
        if eval_bleu:
            ...
            if K>0:
                if mbleu:
                    hyp_paths = []
                    for i in range(K):
                        hyp_name = 'hyp{0}.{1}-{2}.{3}.K{4}.txt'.format(scores['epoch'], lang1, lang2, data_set, i)
                    ...
                    data_root='/data/ldc'
                    if data_set == 'valid':
                        bleus = eval_mbleu(['%s/nist02.tok.ref0'%(data_root), '%s/nist02.tok.ref1'%(data_root), '%s/nist02.tok.ref2'%(data_root), '%s/nist02.tok.ref3'%(data_root)], hyp_paths)
                        logger.info("BLEU (max) %s %s : %f" % (hyp_path, ref_path, max(bleus)))
                        logger.info("BLEU (avg) %s %s : %f" % (hyp_path, ref_path, sum(bleus) / len(bleus)))       
                        scores['%s_%s-%s_mt_bleu' % (data_set, lang1, lang2)] = sum(bleus) / len(bleus)
                    else:
                        bleus = eval_mbleu(['%s/nist03.tok.ref0'%(data_root), '%s/nist03.tok.ref1'%(data_root), '%s/nist03.tok.ref2'%(data_root), '%s/nist03.tok.ref3'%(data_root)], hyp_paths)
                        logger.info("BLEU (max) %s %s : %f" % (hyp_path, ref_path, max(bleus)))
                        logger.info("BLEU (avg) %s %s : %f" % (hyp_path, ref_path, sum(bleus) / len(bleus)))
                        scores['%s_%s-%s_mt_bleu' % (data_set, lang1, lang2)] = sum(bleus) / len(bleus)
                    ...
                    for i in range(K):
                        ...
                        if bleu > best_bleu:
                            best_bleu = bleu
                    logger.info("BLEU %s %s : %f" % (hyp_path, ref_path, best_bleu))
                    scores['%s_%s-%s_mt_bleu' % (data_set, lang1, lang2)] = best_bleu

                div_bleu = eval_diverse_bleu(hyp_paths)
                logger.info("div_BLEU %s %s : %f" % (hyp_path, ref_path, div_bleu))
                scores['%s_%s-%s_mt_div_bleu' % (data_set, lang1, lang2)] = div_bleu
...
def convert_to_diverse_text(batch, lengths, dico, params):
    """
    Convert a batch of sentences to a list of text sentences.
    """
    batch = batch.cpu().numpy()
    ...
    for j in range(bs):
        words = []
        #skip the codes in position 1
        for k in range(2, lengths[j]):
            if batch[k, j] == params.eos_index:
                break
            words.append(dico[batch[k, j]])
        sentences.append(" ".join(words))
...
def eval_mbleu(refs, hyps):
    """
    For ldc dataset
    mBLEU in https://arxiv.org/pdf/2009.09758.pdf
    Given multiple refs and hyps, average/ max the multi-ref bleu
    """
    assert os.path.isfile(BLEU_SCRIPT_PATH)
    for ref in refs:
        assert os.path.isfile(ref) or os.path.isfile(ref + '0')     
    ...
    for hyp in hyps:     
        assert os.path.isfile(hyp)   
        command = BLEU_SCRIPT_PATH + ' %s %s %s %s < %s'
        logger.info("bleu cmd: {}".format(command % (refs[0], refs[1], refs[2], refs[3], hyp)))
        p = subprocess.Popen(command % (refs[0], refs[1], refs[2], refs[3], hyp), stdout=subprocess.PIPE, shell=True)
        result = p.communicate()[0].decode("utf-8")
        if result.startswith('BLEU'):
            b = float(result[7:result.index(',')])
            bleus.append(b)           
        else:
            logger.warning('Impossible to parse BLEU score! "%s"' % result)
            bleus.append(-1)
...
def eval_diverse_bleu(paths):
    """
    Given a file of hypothesis file,
    evaluate the div BLEU score using scarebleu.
    based on pairwise BLEU of https://github.com/pytorch/fairseq/blob/master/examples/translation_moe/score.py
    """
    ...
    for path in paths:
        ...
        with open(path) as f:
            for i, line in enumerate(f):
                if i not in hypos:
                    hypos[i] = []
                hypos[i].append(line.rstrip())
    hypos = dictolist(hypos)
        ...
        for i in range(len(s)):
            for j in range(len(s)):
                if i != j and s[i]!="" and s[j]!="":
                    _ref.append(s[i])
                    _hypo.append(s[j])
    return corpus_bleu(_hypo, [_ref], tokenize="none").score
